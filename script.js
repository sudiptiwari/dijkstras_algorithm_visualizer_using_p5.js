...
function setup() {
  ...
  cnv.position(
    windowWidth / 2 - cnv.width / 2,
    windowHeight / 2 - cnv.height / 2
...
function mouseReleased() {
  ...
  if (
    !checkboxes.nodeCheckbox.checked() &&
    checkboxes.edgeCheckbox.checked() &&
    !checkboxes.shortestPathCheckbox.checked()
  ) {
    if (edge_draw_flag) {
      edge.weight = 0;
      while (
        edge.weight == 0 ||
        edge.weight !== edge.weight /* only NaN is not equal to itself */
      ) {
...
class CheckBoxes {
  constructor(x, y) {
    ...
    this.checkboxes = [
      this.nodeCheckbox,
      this.edgeCheckbox,
      this.startNodeCheckbox,
      this.endNodeCheckbox,
      this.shortestPathCheckbox,
    ];
    this.x = x;
    this.y = y;
...
class Graph {
  ...
  implement_dijkstra() {
    ...
    for (let v = 0; v < this.noOfNodes - 1; v++) {
      ...
      for (let i = 0; i < this.noOfNodes; i++) {
        if (
          !visited[i] &&
          this.edgesMatrix[row][i] != -1 &&
          distance[row] + this.edgesMatrix[row][i] <= distance[i]
        ) {
          // block for step by step highlight of paths being checked
          for (let j = 0; j < edges.length; j++) {
            let e1 = edges[j];
            if (
              (e1.sourceCircle == row && e1.destCircle == i) ||
              (e1.sourceCircle == i && e1.destCircle == row)
            ) {
              stroke(255, 0, 0);
              line(e1.startX, e1.startY, e1.endX, e1.endY);
              // Calculate mid-point for showing e1.weight
    ...
    for (let i = 0; i < this.noOfNodes; i++) {
      ...
      shortestPathEdges.push(
        new Edge(
          circles[parent[temp_src]].x,
          circles[parent[temp_src]].y,
          parent[temp_src],
          circles[temp_src].x,
          circles[temp_src].y,
          temp_src,
  ...
  displayShortestPathEdges() {
    for (let i = 0; i < shortestPathEdges.length; i++) {
      ...
      console.log(
        `Start: (${edge.startX}, ${edge.startY}), Node ${edge.sourceCircle}`
      );
